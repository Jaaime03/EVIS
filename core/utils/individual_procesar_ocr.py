...
def extraer_sql(texto):
    match = re.search(r"```sql\s+(.*?)```", texto, re.DOTALL)
    if match:
...
def individual_procesar_ocr(carpeta_imagenes, enunciado):
    ...
    for nombre_archivo in imagenes:
        ...
        try:
            ...
            texto_extraido = extraer_sql(texto)
            ...
            ejercicio.ocr_imag_to_text = texto_extraido
            ejercicio.correcto_ocr = True
            ejercicio.save()
            
            resultados[id_alumno_str] = {"ejercicio": texto_extraido}
